...
use crate::wallet::key_taint::KeyTaintTracker;
...
pub struct TransactionBuilder {
    ...
    is_multisig: bool,
    multisig_m: u8,
    taint_tracker: KeyTaintTracker,
}

impl TransactionBuilder {
    pub fn new(private_key: PrivateKey, taint_tracker: KeyTaintTracker) -> Self {
        TransactionBuilder {
            inputs: Vec::new(),
            outputs: Vec::new(),
    ...
    pub fn new_multisig(private_keys: Vec<PrivateKey>, m: u8, taint_tracker: KeyTaintTracker) -> Result<Self, Error> {
    ...
    pub fn build(&self) -> Result<Transaction, Error> {
        ...
        let scheme_id = match self.private_keys.first().unwrap() {
            PrivateKey::Ecdsa(_) => {
                warn!("Using ECDSA key, which may be quantum-vulnerable");
                SCHEME_ID_ECDSA
            }
            PrivateKey::Dilithium(_) => SCHEME_ID_DILITHIUM3,
        };
        ...
        for i in 0..self.inputs.len() {
            ...
            for key in &self.private_keys {
                ...
                if self.taint_tracker.is_tainted(&pubkey)? {
                    warn!("Tainted key detected: {:?}", pubkey);
                    return Err(Error::SignatureError("Attempted to use tainted key".to_string()));
                }
                let signature = match key {
                    PrivateKey::Ecdsa(privkey) => {
                        let sig = privkey.sign(&sighash);
                        self.taint_tracker.mark_tainted(&pubkey)?; // Mark ECDSA key as tainted
    ...
    pub fn build_psbt(&self) -> Result<PartiallySignedTransaction, Error> {
        ...
        let scheme_id = match self.private_keys.first().unwrap() {
            PrivateKey::Ecdsa(_) => {
                warn!("Using ECDSA key in PSBT, which may be quantum-vulnerable");
                SCHEME_ID_ECDSA
            }
            PrivateKey::Dilithium(_) => SCHEME_ID_DILITHIUM3,
        };
        ...
        for i in 0..self.inputs.len() {
            ...
            for key in &self.private_keys {
                ...
                if self.taint_tracker.is_tainted(&pubkey)? {
                    warn!("Tainted key detected in PSBT: {:?}", pubkey);
                    return Err(Error::SignatureError("Attempted to use tainted key".to_string()));
                }
                let signature = match key {
                    PrivateKey::Ecdsa(privkey) => {
...
pub fn generate_keypair_and_address(scheme: &str, prefix: Prefix, ext_key: &ExtendedPrivateKey) -> Result<(PrivateKey, Address), Error> {
    match scheme {
        "ecdsa" => {
        ...
        "dilithium3" => {
            let (pubkey, privkey) = DilithiumPrivateKey::from_extended_key(ext_key)?;
            let address = pubkey.to_address(prefix)?;
...
pub fn init() {
    warn!("Wallet transaction module initialized with ECDSA, Dilithium3, multisig, and taint detection");
}
...
mod tests {
    ...
    #[test]
    fn test_tainted_ecdsa_key() {
        let taint_tracker = KeyTaintTracker::new();
        let seed = [0u8; 32];
        let ext_key = ExtendedPrivateKey::new(&seed).unwrap();
        let (privkey, address) = generate_keypair_and_address("ecdsa", Prefix::Testnet, &ext_key).unwrap();
        let pubkey = match &privkey {
            PrivateKey::Ecdsa(pk) => pk.to_public_key().as_bytes().to_vec(),
            _ => panic!("Expected ECDSA key"),
        };
        taint_tracker.mark_tainted(&pubkey).unwrap();
        let mut builder = TransactionBuilder::new(privkey, taint_tracker);
        ...
        assert!(builder.build().is_err(), "Should reject tainted key");
    }

    #[test]
    fn test_multisig_dilithium() {
        let taint_tracker = KeyTaintTracker::new();
        let seed = [0u8; 32];
        let ext_key = ExtendedPrivateKey::new(&seed).unwrap();
        let (privkey1, _) = generate_keypair_and_address("dilithium3", Prefix::Testnet, &ext_key).unwrap();
        let (privkey2, _) = generate_keypair_and_address("dilithium3", Prefix::Testnet, &ext_key).unwrap();
        let mut builder = TransactionBuilder::new_multisig(vec![privkey1, privkey2], 2, taint_tracker).unwrap();
